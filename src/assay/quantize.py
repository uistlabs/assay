...
def load_calibration(cfg: Config, tokenizer):
    ...
    Deliberately takes a tokenizer rather than loading its own from
    cfg.base_model: base_model is the hub identifier while model_path (what's
    actually being quantized) is a local volume path -- if those ever diverge,
    a self-loaded tokenizer here would calibrate with the wrong vocab/chat
    template."""
    from datasets import Dataset, load_dataset

    # streaming=True so we never materialize/cache the whole dataset (ultrachat_200k
    # generates ~515k examples across 4 splits) just to keep num_calibration_samples
    ...
    stream = load_dataset(cfg.calib_dataset, split=cfg.calib_split, streaming=True)
    stream = stream.shuffle(seed=42, buffer_size=10_000)
    ds = Dataset.from_list(list(stream.take(cfg.num_calibration_samples)))
...
def quantize_to_nvfp4(cfg: Config, model_path: str, out_dir: str, hb=None) -> str:
    ...
    ds = load_calibration(cfg, tokenizer)
    recipe = QuantizationModifier(targets="Linear", scheme=cfg.quant_scheme, ignore=["lm_head"])

    if hb:
        hb.emit("quantize", f"oneshot {cfg.quant_scheme} calibration")
    oneshot(
        model=model,
        dataset=ds,
        recipe=recipe,
